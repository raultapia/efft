...
void bind_stimulus(nb::module_ &m) {
...
struct Bindings {
  ...
  Eigen::Matrix<std::complex<float>, N, N, Eigen::RowMajor> get_fft() const {
    return eng.getFFT().template cast<std::complex<float>>().eval();
  }
  int framesize() const { return static_cast<int>(eng.framesize()); }
};

template <unsigned int N>
void bind_efft(nb::module_ &m, const std::string &pyname) {
